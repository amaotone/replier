...
③ フローティングパネルが出現
   ...
   └────────────────────────────────────────┘
        │
④ 指示入力欄にフォーカス。返信の要旨を一言入力してEnterで「短め/長め」2案を横並び生成(空Enterは無操作)。シチュエーション(メール/チャット)とトーンは切替のみで自動再生成しない。意図チップはワンタップ生成のショートカット
⑤ Tabで候補エリアへ → ←→/↑↓で2案切替 → Enterでクリップボードにコピーして閉じる(⌘Enterはどこからでも確定)。Escでキャンセル
※ 日本語IME対応: 入力欄のEnterはonSubmit方式(変換確定Enterでは発火しない)、入力欄フォーカス中は↑↓を奪わない(IMEの候補選択に委譲)
