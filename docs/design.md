...
② グローバルホットキー ⌥⌘R
        │
③ フローティングパネルが出現(取り込んだ文脈は表示しないがLLMには渡す)
   ┌──────────────────────────────────────────────┐
   │ 返信の要旨を一言(Shift+Enterで改行)             │ ← 3行入力・オートフォーカス
   │ ──────────────────────────────────────────── │
   │ 送信先[メール|チャット] トーン[ビジネス|カジュアル]     │
   │ 形式[平文|構造化]      言語[自動|日本語|英語]       │
   │ ──────────────────────────────────────────── │
   │ ┌─短め──────────────┐  ┌─長め──────────────┐  │
   │ │ 承知しました。…      │  │ ご連絡ありがとう…    │  │
   │ └───────────────────┘  └───────────────────┘  │
   └──────────────────────────────────────────────┘
        │
④ 要旨を入力してEnterで「短め/長め」2案を横並び生成(空Enterは無操作)。送信先/トーン/形式/言語は切替のみで自動再生成せず、次のEnterから反映
⑤ Tabで候補エリアへ → ←→/↑↓で2案切替 → Enterでクリップボードにコピーして閉じ、トースト表示(⌘Enterはどこからでも確定)。Escまたはフォーカス喪失で閉じる
※ 日本語IME対応: 入力欄はNSTextViewベースでhasMarkedTextを判定。変換確定Enterでは発火せず、入力欄フォーカス中は↑↓を奪わない(IMEの候補選択に委譲)
⑥ (任意)「この返信を学習」→ 文体プロファイルに蓄積(P2)
```

- 選択テキストが取得できない場合はクリップボード(直前の⌘C)にフォールバック
- 前面アプリを自動判定し、Slackならカジュアル・メールなら丁寧、のアプリ別デフォルトトーンを適用
- 生成は1リクエストで2案(サブスク消費を抑える)。出力は `<<<short>>>`/`<<<long>>>` 区切りテキスト契約で逐次パースし、完成した案から順に確定可能。app-serverは起動時プリウォーム済み
...
| R2 | 生成レイテンシ | app-server常駐でプロセス起動コストは解消済み。残るモデル推論時間は2案1リクエスト+逐次表示+effort minimalで体感を短縮 |
